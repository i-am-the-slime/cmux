...
# Blacksmith or GitHub-hosted label, because the runner variables may name
# owned self-hosted machines. Comparing full_name instead of reading
...
# matrix pool only where it picks the runner.
#
...
OWNED_RUNNER_SELECTOR = re.compile(
    OWNED_RUNNER_VARIABLE.pattern + r"|\bmatrix(?:\.pr_runner\b|\[\s*'pr_runner'\s*\])",
    re.IGNORECASE,
)
